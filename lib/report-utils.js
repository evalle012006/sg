...
export const processAnswer = (answer, questionType = '') => {
    ...
    //    NOTE: keyed-object answers with real domain meaning (service-cards' per-service
    //    { selected, subOptions } map, package/course ids) should already have been
    //    resolved server-side before reaching this function — see lib/server/report-qa-resolver.js.
    //    This branch is a last-resort fallback for anything that slips through unresolved,
    //    and must never let a nested object hit Array.join()/toString() directly
    //    (that's what previously produced literal "[object Object]" cells).
    if (typeof processedAnswer === 'object' && processedAnswer !== null) {
        ...
        if (processedAnswer.title) return processedAnswer.title;
        return Object.entries(processedAnswer)
            .filter(([, v]) => v !== null && v !== undefined && v !== '')
            .map(([k, v]) => {
                if (typeof v === 'object') return null; // unresolved nested object — drop rather than corrupt the cell
