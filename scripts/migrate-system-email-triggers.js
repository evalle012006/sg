 ...
 * 
 * Usage: node scripts/migrate-system-email-triggers.js
 */
...
const SYSTEM_EMAIL_TRIGGERS = [
  ...
  },
  {
    ...
    recipient: 'recipient_type:info', // Uses recipient_type:info from context
    priority: 5,
    enabled: true
  },
...
async function migrateSystemEmailTriggers() {
  ...
  for (const triggerDef of SYSTEM_EMAIL_TRIGGERS) {
    try {
      ...
      if (existing) {
        console.log(`⊘  Skipping: Trigger already exists - ${triggerDef.description}`);
        skipped++;
        continue;
      }
