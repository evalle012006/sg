        ...
        try {
            ...
            if (response.ok) {
                ...
                const fileUrlResponse = await fetch(
                    `/api/storage/upload?filename=${selectedFile.name}&filepath=${props.fileType}`,
                    {
                        method: "GET",
                        headers: {
        ...
        <div className={`mb-2 ${props.className || ''}`}>
            <div 
                className={`flex justify-between items-center text-gray-500 xl:w-96 w-full px-4 py-6 text-base font-normal
                            bg-clip-padding bg-white border border-solid rounded-lg m-0
                            ${props.error ? 'border-2 border-red-500 bg-red-50' : 'border-gray-300'}`}
                onDrop={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
            ...
            {props.error && <p className="mt-1.5 text-red-500">{props.error}</p>}
        </div>)
}
