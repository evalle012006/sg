...
const FileUploadField = (props) => {
    ...
    const handleRename = async (e) => {
        ...
        try {
            ...
            // Step 2: Update QA pair if required props are available
            if (props.questionId && props.sectionId) {
                try {
                    // Build QA pair object matching saveCareDataToAPI pattern
                    const qa_pair = {
                        question: props.question,
                        answer: newName,  // Just the filename, not the URL
                    ...
                    // If updating existing QA pair, include the ID
                    if (props.qaPairId) {
                        qa_pair.id = props.qaPairId;
                    }
                    ...
                    const saveData = {
                        ...
                        flags: {
                            origin: props.origin,
                            bookingUuid: props.bookingUuid || null,
                            pageId: props.pageId,
                    ...
                    const qaPairResponse = await fetch('/api/booking-request-form/save-qa-pair', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                    ...
                    if (!qaPairResponse.ok) {
                        console.error('Failed to update QA pair, but file was renamed');
                    } else {
                        console.log('✅ QA pair updated successfully after file rename');
                    }
                } catch (qaPairError) {
                    console.error('QA pair update error:', qaPairError);
                    // Don't fail the rename if QA pair update fails
                }
            }
                ...
                {status == 'success' && <div className="text-center w-full">
                    <div className="flex items-start">
                        <div className="flex items-start w-full min-w-0">
                            ...
                            <div className="text-left ml-3 w-full min-w-0 flex-1">
                                {file ? (
                                    <React.Fragment>
                                        <p className="m-1 text-sm font-semibold text-slate-700">{file && file.name}</p>
                                    ...
                                    <React.Fragment>
                                        {isRenaming ? (
                                            <form onSubmit={handleRename} onClick={e => e.stopPropagation()}>
                                                <div className="flex items-center gap-2">
                                                    <input
                                                        ...
                                                        className="text-sm font-semibold text-slate-700 border rounded px-2 py-1 flex-1 min-w-[100px] max-w-[180px]"
                                                        onKeyDown={(e) => {
                                                            if (e.key === 'Escape') {
                                                                setIsRenaming(false);
                                                            }
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <button
                                                            ...
                                                            className="px-3 py-1 bg-gray-200 text-gray-700 text-xs rounded hover:bg-gray-300 whitespace-nowrap"
                                                        >
                                                            Cancel
                                                        </button>
                                                        <button 
                                            ...
                                            <div className="flex items-center gap-2">
                                                <p className="m-1 text-sm font-semibold text-slate-700 underline cursor-pointer truncate max-w-[180px]" 
