            ...
            <div className="flex items-center justify-center h-screen">
                <div className="w-96 max-w-full h-auto bg-white drop-shadow-md  rounded-lg flex flex-col justify-center" onClick={(e) => e.stopPropagation()}>
                    <div className="py-10 px-8">
                        <div>
                            <Input inputClass={editMode ? '' : 'text-gray-500 bg-gray-100'} onFocus={() => setEditMode(true)} label={'Email'} onChange={(e) => setFormState({ ...formState, notification_to: e.target.value })} value={formState.notification_to} name={'notification_to'} />
                        </div>
                    </div>
